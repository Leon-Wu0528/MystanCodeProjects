...
def get_average(pixels):
    ...
    for i in range(len(pixels)):
        ...
        red += pixels[i].red
        blue += pixels[i].blue
    avg.append(int((red/n)//1))
    avg.append(int((green/n)//1))
    avg.append(int((blue/n)//1))
    return avg
...
def get_best_pixel(pixels):
    ...
    for i in range(len(pixels)):
        temp = get_pixel_dist(pixels[i], avg_red, avg_green, avg_blue)
        if temp <= best_dis:
            best_dis = temp
            best_pixel = pixels[i]
    return best_pixel
    ...
    for x in range(result.width):
        for y in range(result.height):
            ...
            for i in range(len(images)):
                ...
                select.append(pixel)
            best_pixel = get_best_pixel(select)
